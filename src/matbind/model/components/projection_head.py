...
ACTIVATION_RESOLVER = ClassResolver(
    [nn.ReLU, nn.LeakyReLU, nn.Sigmoid, nn.Tanh, nn.GELU], base=nn.Module, default=nn.ReLU
)
...
class ProjectionHead(nn.Module):
    ...
    def _build_projection_head(
        ...
        for i in range(len(dims) - 1):
            ...
            if i == len(dims) - 2:
                # according to CLIP, no bias in the last layer
                layers.append(nn.Linear(dims[i], dims[i + 1], bias=False))
            else:
                layers.append(nn.Linear(dims[i], dims[i + 1]))
            # Apply activation function if not the last layer
            if i < len(dims) - 2:
                # for intermediate layers always apply layer norm + activation
...
if __name__ == "__main__":
    # Simple test
    model = ProjectionHead(dims=[128, 64, 32], activation="relu")
    print(model)